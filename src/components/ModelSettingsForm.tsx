...
import { Loader2, AlertTriangle, Cloud, HardDrive, Zap, CheckCircle } from "lucide-react";
...
const ModelSettingsForm = () => {
  const { settings, updateSettings, isLoading: contextLoading } = useModelSettings();
  ...
  // Load settings when context is ready
  useEffect(() => {
  ...
  const handleProviderChange = (provider: ModelProvider) => {
    const newSettings = { ...settings, provider, selectedModel: undefined };
  ...
  const handleLocalUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newSettings = { ...settings, localModelUrl: url };
    updateSettings(newSettings);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-green-50 dark:bg-green-950/20 p-4 rounded-lg border">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-medium">Current Model Configuration</span>
        </div>
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2">
            <span>Provider:</span>
            <Badge variant="secondary" className="capitalize">{settings.provider}</Badge>
          </div>
        ...
        <p className="text-xs text-muted-foreground mt-2">
          This configuration affects all AI features across ZeroLoop including learning loops, 
          knowledge search, AI agent chat, and reasoning engines.
        </p>
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-950/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
        <h4 className="font-medium text-blue-900 dark:text-blue-100 mb-2">Global Configuration</h4>
        <p className="text-sm text-blue-800 dark:text-blue-200">
          Changes to model settings will immediately affect all AI-powered features throughout ZeroLoop, 
          including the AI Agent, Learning Loops, Knowledge Search, and Reasoning Engines.
        </p>
      </div>
    </div>
  );
